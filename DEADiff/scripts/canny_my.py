...
# 确保项目目录下有 ldm/controlnet/annotator 文件夹
...
class DEADiff_CannyControl(object):
    def __init__(self,
        ...
        # 检查模型路径是否存在，打印提示
        if not os.path.exists(ckpt_controlnet):
            print(f"Warning: ControlNet model not found at {ckpt_controlnet}")
    ...
    def generate(self, prompt, image_style_input, image_content_input, subject_text, batch_size, ddim, ddim_steps, scale, img_weight, seed):
        # ---------------- 【修复 1：输入检查】 ----------------
        if image_style_input is None:
            print("【错误】未上传 Style Image (参考风格图)！")
            return []
        if image_content_input is None:
            print("【错误】未上传 Content Image (边缘控制图)！")
            return []

        # ---------------- 【修复 2：随机种子处理】 ----------------
        # 处理 -1 种子的情况，避免 numpy 报错
        if seed == -1:
            seed = np.random.randint(0, 2147483647)
        
        print(f"DEBUG: Generating with Seed: {seed}")
        ...
        with torch.no_grad():
            with precision_scope("cuda"):
                with self.model.ema_scope():
                    ...
                    ####### depth control #######
                    # ---------------- 【修复 3：数据清洗 (关键)】 ----------------
                    # 强制转换为 uint8 类型，解决 OpenCV 类型不匹配问题
                    # 强制内存连续 (ascontiguousarray)，解决 OpenCV 内存报错
                    content_img_safe = np.array(image_content_input).astype(np.uint8)
                    content_img_safe = np.ascontiguousarray(content_img_safe)
                    
                    # 传入清洗后的数据，并额外加一个 HWC3 处理
                    img = resize_image(HWC3(content_img_safe), 384)
                    H, W, C = img.shape
                    # --------------------------------------------------------
...
def interface(bd_inference):
    ...
    with gr.Row():
        # ---------------- 【修复 4：Batch Size 默认值】 ----------------
        # 将 value 改为 1，防止显存 OOM
        batch_size = gr.Slider(1, 8, value=1, step=1, label="出图数量(batch_size)")
        # -------------------------------------------------------------
    ...
    inputs=[
            prompt,
            image_style_input,
            image_content_input, # 确保 content image 在输入列表里
            subject_text,
            batch_size,
            ddim,
            ddim_steps,
...
if __name__ == "__main__":
    # 请确保这里的最后一个参数 (ControlNet路径) 和你实际下载的文件名一致
    # 常见名字：control_v11p_sd15_canny.pth 或 control_sd15_canny.pth
